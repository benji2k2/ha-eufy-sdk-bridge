...
| `STREAM_LINGER_MS` | `20000` (20 s) | when the requester gives up **while** the camera wakes (HA and HomeKit allow ~5 s, a battery camera needs ~7 s), keep the woken session this long after its first bytes so the requester's retry joins an awake camera instead of waking it again; then release it. `0` releases at once |
