...
| `STREAM_FIRST_DATA_MS` | `25000` (25 s) | answer `/stream` only once the camera delivers its first bytes, waiting up to this long. A battery camera needs several seconds to wake; answering earlier hands the consumer an empty stream, which it rejects with "Invalid data found". A camera that stays silent this long fails the request and arms the backoff. `0` answers at once (old behaviour) |
| `STREAM_LINGER_MS` | `20000` (20 s) | when the requester gives up **while** the camera wakes (HA's first attempt typically errors out a couple of seconds before a battery camera delivers), keep the woken session this long after its first bytes so the requester's retry joins an awake camera instead of waking it again; then release it. `0` releases at once |
